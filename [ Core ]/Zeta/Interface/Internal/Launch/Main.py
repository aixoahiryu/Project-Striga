...
class Launch(Window):
	def __init__(self, *args, **kwargs):
		...
		Button(frame0, text='[ Scraps ]', command=lambda: os.startfile(ZLCORE+r'\Toolbar\_\[ Program ]\[ Source ]\[ Scraps ].txt')).grid(sticky='W', column=0, row=0)
		Button(frame0, text='Active', command=lambda: os.startfile(ZLCORE+r'\Toolbar\_\[ Program ]\[ Source ]\Active.txt')).grid(sticky='W', column=1, row=0)

		frame1 = Frame(self.frame)
		frame1.grid(sticky='S', column=0, row=1)
		Button(frame1, text='Proxy', command=lambda: os.startfile(ZLCORE+r'\Toolbar\_\[ Program ]\[ Source ]\Proxy.py')).grid(sticky='N', row=0)
		...
		Button(frame3, text='Experiment', command=lambda: os.startfile(ZLCORE+r'\Toolbar\_\[ Program ]\[ Source ]\Experiment.txt')).grid(sticky='W', row=0, column=0)
		Button(frame3, text='File', command=lambda: os.startfile(ZLCORE+r'\Toolbar\_\[ Program ]\[ Source ]\File.txt')).grid(sticky='W', row=0, column=1)
		...
		linkbtn = Button(self.frame, text='[ Link ]', command=lambda: os.startfile(ZLCORE+r'\Toolbar\_\[ Program ]\[ Source ]\Link.py'))
